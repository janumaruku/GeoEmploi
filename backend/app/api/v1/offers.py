...
from app.schemas.offer import OfferCreate, OfferRead, OfferStatusUpdate, OfferUpdate

router = APIRouter()


@router.post("", response_model=OfferRead, status_code=status.HTTP_201_CREATED)
def create_offer(data: OfferCreate, db: Session = Depends(get_db), current_user: User = Depends(require_role(UserRole.EMPLOYER))):
...
def update_offer(offer_id: int, data: OfferUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    offer = crud_offer.get_offer(db, offer_id)
    if not offer:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Offer not found")
...
def update_offer_status(offer_id: int, data: OfferStatusUpdate, db: Session = Depends(get_db), _: User = Depends(require_role(UserRole.ADMIN))):
    offer = crud_offer.get_offer(db, offer_id)
    if not offer:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Offer not found")
