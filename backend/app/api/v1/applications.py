...
def list_applications(offer_id: int | None = None, applicant_id: int | None = None, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if current_user.role == UserRole.JOB_SEEKER:
...
def update_application_status(application_id: int, data: ApplicationStatusUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    application = crud_application.get_application(db, application_id)
    if not application:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Application not found")
    offer = crud_offer.get_offer(db, application.offer_id)
