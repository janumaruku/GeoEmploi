  ...
  return (
    <article className="offer-card">
      ...
      <dl className="offer-details">
        ...
        <div><span className="detail-icon" aria-hidden="true">◷</span><dt>Contrat</dt><dd>{offer.contract}</dd></div>
      </dl>
      <div className="offer-section">
        ...
        <ul className="offer-missions">
          {offer.missions.map((mission) => <li key={mission}>{mission}</li>)}
        </ul>
        <h4>Profil recherché</h4>
        <p>{offer.profile}</p>
        <button type="button" className="apply-button" onClick={handleApply}>Postuler</button>
        {showAuthChoice && !isAuthenticated && (
          <div className="apply-auth" role="status">
            <p>Connectez-vous ou créez un compte pour postuler.</p>
            <div>
              <button type="button" onClick={() => { setShowAuthChoice(false); onLogin() }}>Se connecter</button>
              <button type="button" onClick={() => { setShowAuthChoice(false); onRegister() }}>S’inscrire</button>
            </div>
          </div>
        )}
      </div>
